...
@WebServlet("/applications/status")
public class UpdateApplicationStatusServlet extends AppServlet {
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        ApplicationStatus status = ApplicationStatus.from(rawStatus).orElse(null);
        if (status == null) {
            setFlash(req, "error", "Invalid application status.");
            redirect(req, resp, resolveRedirectPath(req, jobId));
            return;
        }

        ActionResult result = services(req).applicationService().updateStatus(applicationId, status, user);
